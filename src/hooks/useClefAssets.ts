...
// 画像の読み込み
const imageFilesTreble: Record<string, { default: string }> = import.meta.glob(
	"../assets/images/clef_treble/*.png",
	{ eager: true },
);
const imagesTreble = Object.fromEntries(
	Object.entries(imageFilesTreble).map(([key, value]) => {
		const name =
			key.match(/\..\/assets\/images\/clef_treble\/(.+)\.png$/)?.[1] || "";
		return [name, value.default];
	}),
);

const imageFilesBass: Record<string, { default: string }> = import.meta.glob(
	"../assets/images/clef_bass/*.png",
	{ eager: true },
);
const imagesBass = Object.fromEntries(
	Object.entries(imageFilesBass).map(([key, value]) => {
		const name =
			key.match(/\..\/assets\/images\/clef_bass\/(.+)\.png$/)?.[1] || "";
		return [name, value.default];
	}),
);
...
export function useClefAssets(clefType: "treble" | "bass") {
	...
	const images = useMemo(
		() => (clefType === "treble" ? imagesTreble : imagesBass),
		[clefType],
	);
	...
	// 音階データ
	// NOTE: 内容は一緒だけどヘ音記号の場合はE始まりからにしたいので条件分岐で並び順の違う配列を作っている。
	const notes =
		clefType === "treble"
			? [
